defmodule TextMessengerBackend.ChatService.SqsClient do
  require Logger
  alias TextMessengerBackend.ChatService.AWS.SignatureV4
  ...
  def build_request(queue_url, message_body) do
    host = Application.get_env(:chat_service, :sqs)[:host]
    url = Application.get_env(:chat_service, :sqs)[:url]
    session_token = Application.get_env(:chat_service, :aws)[:session_token]
    ...
    credentials = %{
      access_key: Application.get_env(:chat_service, :aws)[:access_key],
      secret_key: Application.get_env(:chat_service, :aws)[:secret_key],
